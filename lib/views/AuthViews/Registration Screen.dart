...
class _RegistrationScreenState extends State<RegistrationScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: authRepoProvider.isLoading? Center(child: CircularProgressIndicator()): SingleChildScrollView(
        child: Padding(
                  ...
                  child: ElevatedButton(
                      ...
                      onPressed: () async {
                        if (formKey.currentState!.validate()) {
                          ...
                          var result = await authRepoProvider.userRegister(
                              username: userNameController.text,
                              email: emailController.text,
                              phone: mobileNumberController.text,
                              password: passwordController.text);
                          // print(jsonEncode(result));
                          print('status: ${authRepoProvider.registerData[0].status}');
                          if (authRepoProvider.registerData[0].status == 1) {
                            print('Hello');
                            String userId = authRepoProvider.registerData[0].data.id;
                            // print(jsonEncode(result));
                            print(userId);
